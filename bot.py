...
def add_user(user, lang="uz"):
    ...
    if exists:
        ...
        cur.execute(
            """
            INSERT INTO users (telegram_id, full_name, username, language, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
...
def update_cashback_status(request_id, status, amount=0):
    conn = db()
    cur = conn.cursor()
...
async def cashback_approve(callback: CallbackQuery):
    ...
    item = pending_cashbacks.get(request_id)
    if not item:
        await callback.answer("Ariza topilmadi yoki bot qayta ishga tushgan.", show_alert=True)
        return
...
async def cashback_reject(callback: CallbackQuery):
    ...
    item = pending_cashbacks.pop(request_id, None)
...
async def cashback_paid(callback: CallbackQuery):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer("Ruxsat yo‘q.", show_alert=True)
        return
    ...
    update_cashback_status(request_id, "paid", 0)
    lang = item["lang"]
    ...
    await callback.message.answer("💳 Cashback to‘landi deb belgilandi.")
    await callback.answer()
...
async def complaint_problem_image(message: Message, state: FSMContext):
    ...
    pending_complaints[complaint_id] = {
        "telegram_id": message.from_user.id,
        "lang": lang,
        "data": final_data,
    }
...
async def complaint_reply(callback: CallbackQuery, state: FSMContext):
    ...
    if complaint_id not in pending_complaints:
        await callback.answer("Shikoyat topilmadi yoki bot qayta ishga tushgan.", show_alert=True)
        return
...
async def admin_custom_reply(message: Message, state: FSMContext):
    ...
    item = pending_complaints.get(complaint_id)

    if not item:
        await message.answer("Shikoyat topilmadi yoki bot qayta ishga tushgan.")
        await state.clear()
        return
...
async def complaint_done(callback: CallbackQuery):
    ...
    complaint_id = callback.data.split(":")[1]
    item = pending_complaints.pop(complaint_id, None)
...
async def template_lang(callback: CallbackQuery):
    ...
    pending_admin_replies[complaint_id] = {
        "text": text,
        "lang": lang,
    }

    await callback.message.answer(
        "Shu javobni yuboramizmi?\n\n"
        f"{text}",
...
async def template_send(callback: CallbackQuery):
    ...
    item = pending_complaints.get(complaint_id)
    reply = pending_admin_replies.get(complaint_id)

    if not item or not reply:
        await callback.answer("Ma’lumot topilmadi yoki bot qayta ishga tushgan.", show_alert=True)
        return
...
async def template_edit(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "✏️ Tahrirlangan javob matnini yuboring:"
    )
...
async def admin_panel(message: Message):
    ...
    cur.execute("SELECT COUNT(*) FROM cashback_requests")
    cashback_count = cur.fetchone()[0]

    cur.execute("SELECT COUNT(*) FROM complaints")
    complaints_count = cur.fetchone()[0]

    cur.execute("SELECT COUNT(*) FROM cashback_requests WHERE status = 'paid'")
    ...
    await message.answer(
        ...
        f"💰 Cashback arizalari: {cashback_count}\n"
        f"💳 To‘langan cashbacklar: {paid_count}\n"
        f"📝 Shikoyatlar: {complaints_count}"
    )
...
async def main():
    init_db()
    print("Uzum Cashback Bot ishga tushdi...")
