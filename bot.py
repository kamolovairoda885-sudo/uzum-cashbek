...
async def admin_check_photo(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    request_id = data.get("cashback_request_id")
    item = get_cashback_request(request_id)
    ...
    # Tasdiqlangan summani saqlab, statusni paid ga o'tkazamiz
    update_cashback_status(request_id, "paid", item["amount"])
    lang = item["language"]
    ...
    try:
        ...
        if message.photo:
            await bot.send_photo(item["telegram_id"], message.photo[-1].file_id)
        elif message.document:
            await bot.send_document(item["telegram_id"], message.document.file_id)
    except Exception as e:
